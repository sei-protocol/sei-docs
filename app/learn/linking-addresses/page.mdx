...
Each Sei address in Sei V2 has a corresponding EVM / 0x address, and vice versa.
When you use Sei V2 for the first time, it is necessary to link these two
addresses.

<Callout type="warning">**Important Note:** You cannot decide which Sei and 0x addresses are linked together. Each Sei account has exactly one specific Sei address, and one specific EVM / 0x address, associated with it. They are like 2 sides of the same coin, and cannot be combined with other addresses.</Callout>
...
Every account on Sei has a unique public key. In Sei V2, this public key
translates into two different types of addresses:

- **EVM Address**: This address starts with `0x` and is used for EVM-based
  activities.

- **SEI Address**: This address starts with `sei` and is used for Sei-native
  activities.
