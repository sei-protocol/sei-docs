...
const EcosystemApps = () => {
		...
		() =>
			appData.filter(
				(app) =>
					...
					app.tags.some((tag) => tagPrettyNames[tag]?.some((prettyName) => prettyName.toLowerCase().includes(searchTerm.toLowerCase())))
			),
		[searchTerm]
	);
	...
	return (
		<div>
			...
			/>
			<div className='mt-2 mb-4 text-sm text-gray-600'>{allTags.join(', ')}</div>
			<EcosystemCards>
				{filteredApps.length > 0 ? (
					filteredApps.map((app) => <EcosystemCard key={app.title} title={app.title} description={app.description} href={app.href} image={app.image} />)
				) : (
