...
export function IbcChannelList() {
	const [expandedChains, setExpandedChains] = useState<string[]>([]);

	function toggleChain(chainName: string) {
		if (expandedChains.includes(chainName)) {
			setExpandedChains(expandedChains.filter((c) => c !== chainName));
		} else {
			setExpandedChains([...expandedChains, chainName]);
		}
	}
