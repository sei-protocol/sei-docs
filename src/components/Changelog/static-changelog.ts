/**
 * Hardcoded changelog entries for stable, older versions (v5.9.0 and below).
 * These entries never change upstream, so we avoid re-fetching and re-parsing
 * them from the remote CHANGELOG.md on every build.
 */
...
export const STATIC_CHANGELOG: StaticChangelogEntry[] = [
	{
		version: 'v5.9.0',
		body: `sei-chain
...
export const STATIC_VERSION_NAMES = new Set(STATIC_CHANGELOG.map((entry) => entry.version));
