...
export default function AppCardV2({ app, title, description, href, icon, logoUrl: logoUrlOverride }: AppCardV2Props) {
	...
	return (
		<div className='group relative overflow-hidden transition-all duration-300 ease-out transform-gpu will-change-transform hover:scale-[1.02] hover:shadow-xl hover:shadow-black/10 dark:hover:shadow-black/30 rounded-sm border backdrop-blur-sm bg-neutral-50/80 dark:bg-neutral-900/80 border-neutral-200/50 dark:border-neutral-800/50 hover:bg-white dark:hover:bg-neutral-900 hover:border-neutral-300 dark:hover:border-neutral-700 p-5 h-full flex flex-col'>
			...
			<div className='absolute inset-0 rounded-sm opacity-0 group-hover:opacity-100 transition-all duration-700 bg-gradient-to-r from-[#34050d]/15 via-[#34050d]/10 to-[#34050d]/15 dark:from-[#600014]/20 dark:via-[#600014]/15 dark:to-[#600014]/20 blur-sm -z-10 transform scale-105' />

			<div className='flex flex-col gap-4 relative z-10'>
				<div className='flex items-start justify-between'>
					<figure className='w-12 h-12 rounded-sm overflow-hidden flex-shrink-0 bg-neutral-200/50 dark:bg-neutral-800/50 group-hover:bg-[#34050d]/10 dark:group-hover:bg-[#600014]/20 transition-all duration-300 group-hover:scale-110 group-hover:rotate-3 flex items-center justify-center'>
						{logoUrl ? (
							<Image
								src={logoUrl}
								alt={name || ''}
							...
							icon || <IconWorld className='text-neutral-500 group-hover:text-[#600014] transition-colors duration-300' size={24} />
						)}
					</figure>
				</div>

				<div className='flex flex-col gap-2 flex-grow'>
					<h3 className='text-base font-bold text-neutral-800 dark:text-neutral-200 group-hover:text-[#34050d] dark:group-hover:text-[#b99ba1] transition-colors duration-300 leading-tight'>
						{name}
					</h3>
				...
				{finalIntegrationLink && (
					<div className='flex items-center mt-auto pt-2'>
						<a
							...
							className='inline-flex items-center gap-1.5 text-sm font-medium text-[#34050d] dark:text-[#b99ba1] hover:text-[#34050d] dark:hover:text-[#e6d4c3] transition-colors duration-200 group/link'>
							<span>{linkText}</span>
				...
				<div className='w-full h-0.5 bg-gradient-to-r from-transparent via-[#34050d]/20 dark:via-[#b99ba1]/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300' />
			</div>
		</div>
	);
}
