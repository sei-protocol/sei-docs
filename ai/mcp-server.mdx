...
<Warning>Generate a dedicated wallet for MCP operations. Never use your main wallet's private key. Wallet mode is blocked when you use an HTTP transport.</Warning>
...
| `get_address_from_private_key` | Retrieve wallet address | "What's my wallet address?" | Required |
| `get_balance` | Check SEI balance | "Check balance of 0x123..." | No |
| `transfer_sei` | Send SEI tokens | "Send 1 SEI to 0x456..." | Required |
| `is_contract` | Verify contract address | "Is 0x789... a contract?" | No |
...
| Tool | Purpose | Example | Wallet mode |
| --- | --- | --- | --- |
| `get_token_info` | Token metadata | "Get USDC token info" | No |
| `get_token_balance` | Token balance | "Check my USDC balance" | No |
| `transfer_token` | Token transfer | "Send 100 USDC to 0x123..." | Required |
| `approve_token_spending` | Token approval | "Approve DEX for USDC" | Required |
...
| Tool | Purpose | Example | Wallet mode |
| --- | --- | --- | --- |
| `get_nft_info` | NFT metadata | "Show NFT #123 details" | No |
| `check_nft_ownership` | Ownership verification | "Who owns NFT #456?" | No |
| `transfer_nft` | NFT transfer | "Send NFT #789 to 0xABC..." | Required |
| `get_nft_balance` | Collection balance | "How many NFTs do I own?" | No |
...
| Tool | Purpose | Example | Wallet mode |
| --- | --- | --- | --- |
| `get_chain_info` | Network information | "Show Sei mainnet info" | No |
| `get_block_by_number` | Block details by number | "Get block 12345" | No |
| `get_latest_block` | Latest block details | "Get latest block" | No |
| `get_transaction` | Transaction data | "Show tx 0xTXID..." | No |
| `read_contract` | Contract state | "Read DEX reserves" | No |
...
Use the Streamable HTTP transport for an HTTP client:
...
Connect to `http://localhost:8080/mcp`. You can change the endpoint with `SERVER_HOST`, `SERVER_PORT`, and `SERVER_PATH`.

<Warning>HTTP transports reject wallet mode. Use the default stdio transport for tools that sign transactions.</Warning>
...
**Private key errors**: Set `WALLET_MODE=private-key`, ensure the key starts with `0x`, and use the default stdio transport.
