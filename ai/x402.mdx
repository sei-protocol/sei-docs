...
Sei EVM also lets you use the upstream `@x402` packages and standard EVM wallet tooling. Native USDC is already included in the x402 default asset registry for Sei Mainnet and Sei Testnet, so dollar-denominated route pricing works without a custom token mapping.

<Info>
When you configure x402 on Sei:

- Sei Mainnet uses the CAIP-2 network identifier `eip155:1329`.
- Sei Testnet uses the CAIP-2 network identifier `eip155:1328`.
...
| --- | --- | --- |
| Sei Mainnet | `eip155:1329` | `0xe15fC38F6D8c56aF07bbCBe3BAf5708A2Bf42392` |
...
The following x402 v2 example charges `0.001` USDC for `GET /weather` on Sei Testnet. Set `X402_FACILITATOR_URL` to a facilitator that supports `eip155:1328`.
...
- Confirm facilitator support for Sei Mainnet or Sei Testnet before you deploy.
