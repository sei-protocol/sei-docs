...
<Card horizontal title="In Development" icon="clock">
  <Columns cols={2}>
    <Card horizontal title="Block Size Constraints">
      ...
      | Max gas per block | Derived from genesis `consensus_params.block.max_gas` (`MaxGasPerBlock`) |
    ...
    <Card horizontal title="Enforcement">
      ...
      - The block gas limit is sourced from genesis `consensus_params.block.max_gas`; genesis must set this to a value greater than zero or the node will refuse to start.
