...
RPC node operators who want to split EVM historical state into a dedicated database should follow the [Giga SS Store Migration Guide](/node/giga-storage-migration). That flow is RPC-only and requires a full state sync.
