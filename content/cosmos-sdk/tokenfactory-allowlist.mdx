...
The [Sei Token Factory module](/cosmos-sdk/tokenfactory-tutorial)
...
- `--chain-id arctic-1`: This specifies the network where the command will be
  executed. In this case, `arctic-1` is the identifier for the Sei devnet.
- `--node https://rpc.arctic-1.seinetwork.io/ `: This points to the RPC URL of
...
- **Valid Addresses**: All addresses in the allowlist must be valid in their
  respective formats (SEI Bech32 or EVM 0x prefixed). Invalid addresses will
  result in a validation error.
- **Address Conversion**: EVM addresses (0x prefixed) are automatically
  converted to SEI Bech32 addresses when the allowlist is processed.
...
1.  First, create a JSON file that contains the list of addresses you want to
    ...
    > Ensure that all addresses are valid SEI (Bech32) or EVM (0x-prefixed)
...
[<ins>Token Factory tutorial</ins>](cosmos-sdk/tokenfactory-tutorial.mdx#create-pointer-contract).
