...
**Lemma 1 (Deterministic Execution):** Given the same initial state (S_init) and the same ordered sequence of transactions (tx₁, tx₂, ..., txₙ), all honest nodes executing these transactions will arrive at the exact same final state (S_final).
...
Data dissemination occurs through independent lanes. Each replica `r` maintains its own lane `ℓr` where it sequentially proposes batches of transactions (ℓr₀, ℓr₁, ℓr₂, ...).

The proposal and certification process follows three steps:
1. **Propose:** Replica `r` creates a data proposal `Prop = ⟨pos, B, parentRef⟩ᵣ` containing position, transaction batch, and a reference to the previous proposal
2. **Vote:** Replicas receiving a valid `Prop` return a vote `Vote = ⟨digest(Prop)⟩`
...
1. **Prepare:** The leader for the current slot collects the latest certified tips from all lanes, forms a `Cut = {ℓ₁[tip], ..., ℓₙ[tip]}`, and broadcasts it
...
For a block containing transactions t₁, t₂, ..., tₙ, where each transaction reads from a set Rᵢ and writes to a set Wᵢ of state variables:
- A dependency `tⱼ → tᵢ` exists if `j > i` and Wᵢ intersects with `Rⱼ ∪ Wⱼ`
...
- **Resource Vector:** Each transaction `t` has a usage vector r_t = (r_t(cpu), r_t(storage), r_t(io), ...)
- **Price Vector:** The network maintains a dynamic price vector p = (p_cpu, p_storage, p_io, ...)
- **Fee Calculation:** Fee(t) = Σ (p_dimension * r_t(dimension)) + F_priority
