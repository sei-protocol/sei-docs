...
Query oracle prices using Sei's custom bindings:

```rust
use cosmwasm_std::{Deps, StdResult, StdError, Decimal};
...
pub fn query_exchange_rate(
    ...
    let rates_response = querier.query_exchange_rates()?;
    
    rates_response
        .denom_oracle_exchange_rate_pairs
        .iter()
        .find(|pair| pair.denom == denom)
        .map(|pair| pair.oracle_exchange_rate.exchange_rate)
...
pub fn query_twap(
    ...
    let querier = SeiQuerier::new(&deps.querier);
    let twaps_response = querier.query_oracle_twaps(lookback_seconds)?;
    
    twaps_response
        .oracle_twaps
        .iter()
        .find(|twap| twap.denom == denom)
        .map(|twap| twap.twap)
        .ok_or_else(|| StdError::generic_err("TWAP not found for denom"))
}
```
