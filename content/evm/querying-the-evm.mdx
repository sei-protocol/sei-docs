...
<Callout type="info" emoji={<IconInfoCircle className="h-5 w-5 mt-1" />}>
  **Quick Navigation:**
    ...
    - [What is EVM Querying?](#what-is-evm-querying)
    - [Prerequisites](#prerequisites)
    - [Network Configuration](#network-configuration)
    - [Address Mapping](#address-mapping)
    - [ERC20 Contract Queries](#erc20-contract-queries)
    - [Payload Generation](#payload-generation)
    - [Pointer Contract Lookup](#pointer-contract-lookup)
...
<Callout type="info" emoji={<IconInfoCircle className="h-5 w-5 mt-1" />}>
  Refer to the [RPC endpoints](/rpc-endpoints) for a list of available Sei Cosmos RPC endpoints.
</Callout>
...
```bash copy
seid q evm payload ./erc20.abi.json transfer 0xrecipient... 1000
```

<Callout type="info" emoji={<IconInfoCircle className="h-5 w-5 mt-1" />}>
  This is useful for manually crafting `call-contract` transactions.
</Callout>
