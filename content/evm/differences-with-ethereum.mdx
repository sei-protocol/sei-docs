...
<Callout type="info">Interoperability between EVM and Cosmos-SDK modules is governed and navigated via [precompiles](./precompiles/example-usage) and [pointer contracts](/learn/pointers)</Callout>
  ...
  <table className="w-full divide-y divide-neutral-200 dark:divide-neutral-800">
    ...
    <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
      ...
      <tr>
        ...
        <td className="px-4 py-3 text-sm text-green-600 dark:text-green-400">Returns current dynamic base fee; no burn</td>
        ...
        <td className="px-4 py-3 text-sm text-neutral-600 dark:text-neutral-400">Legacy tx must specify ≥ 1 gwei base fee on Sei.</td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td className="px-4 py-3 text-sm text-green-600 dark:text-green-400">Not supported (Cancun without blobs)</td>
...
When deploying existing Ethereum contracts to Sei:

- Re-deploy your Solidity code to Sei testnet; most contracts require no changes.
- If you used `SELFDESTRUCT`, refactor to a soft-close pattern.
- Simplify fee UI: use a single `gasPrice` input instead of EIP-1559 complexity.
- For on-chain randomness, integrate an oracle or VRF solution.
- Set `gasLimit` with a modest buffer; parallel execution can cause slight estimate variance.
