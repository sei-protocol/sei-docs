...
Obtain your Sei API key from [Seitrace](https://seitrace.com/auth/profile/api-keys) and replace YOUR_API_KEY placeholder below with your actual API key.
...
```bash copy
npx hardhat ignition modules \scripts\<path>\deploy.multicall.ts --network sei_atlantic_1
```
