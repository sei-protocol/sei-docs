...
3. **Voting Period:** 5 days of active voting once minimum deposit is met
...
| ----------------------- | --------- | ----------------------------------- |
| **Minimum Deposit**     | 3,500 SEI | Required to move proposal to voting |
| **Expedited Threshold** | 7,000 SEI | Enables faster processing           |
| **Deposit Period**      | 2 days    | Time to reach minimum deposit       |
| **Voting Period**       | 5 days    | Duration for casting votes          |
| **Quorum**              | 33.4%     | Minimum participation required      |
| **Veto Threshold**      | 33.4%     | NoWithVeto votes to burn deposit    |

### Proposal Types

- **Text Proposals:** General governance decisions
- **Parameter Change:** Modify protocol parameters
- **Community Pool Spend:** Allocate community funds
- **Software Upgrade:** Network upgrade proposals
...
Add Sei Mainnet to your MetaMask or configure your provider:
...
| NoWithVeto | 4     | Vote against with veto        | Burns deposit if >33.4%          |
...
<Callout type="warning">**Important Deposit Requirements:** - **Minimum deposit:** 3,500 SEI required for proposal to enter voting period - **Expedited processing:** 7,000 SEI enables faster timeline - **Deposit period:** 2 days to meet minimum or proposal fails and deposits are burned - **Multiple deposits:** Anyone can contribute to reach the minimum threshold</Callout>
...
```bash copy
npm install ethers @sei-js/evm@^2.0.0 dotenv
```
...
| --------------------------- | --------------------------------------- | ------------------------------------------------------------ |
| `proposal does not exist`   | Invalid proposal ID or proposal expired | Verify proposal ID and check if voting period is active      |
| `inactive proposal`         | Voting/deposit period ended             | Check proposal status and timeline                           |
| `insufficient deposit`      | Deposit below 3,500 SEI                 | Increase deposit to minimum required amount                  |
| `invalid vote option`       | Vote option not 1-4                     | Use valid vote option (1=Yes, 2=Abstain, 3=No, 4=NoWithVeto) |
| `already voted`             | Already voted on proposal               | Each address can only vote once per proposal                 |
| **`weights must sum to 1`** | **Weighted votes don't sum to 1.0**     | **CRITICAL:** Ensure weights sum to exactly 1.0              |
| `insufficient voting power` | No staked SEI                           | Delegate SEI to validators for voting power                  |
| `deposit period ended`      | Cannot add deposits anymore             | Deposit period is only 2 days after submission               |
...
- **Minimum Deposit:** 3,500 SEI minimum (approximately $3,500+ USD at current prices)
- **Deposit Risk:** Deposits can be burned if proposal receives >33.4% NoWithVeto votes
- **Irreversible Votes:** Once cast, votes cannot be changed
- **Voting Power Required:** Must stake SEI with validators to participate
- **Time Limits:** Strict 2-day deposit period and 5-day voting period
- **Real Impact:** Successful proposals change network parameters and protocols

</Callout>

<Callout type="info">**Deposit Requirements Source:** These amounts are confirmed in the [official Sei governance documentation](https://docs.sei.io/learn/general-governance), which states that the minimum deposit is 3,500 SEI with an expedited threshold of 7,000 SEI.</Callout>
...
- **Documentation:** [Official Sei Docs](https://docs.sei.io)
