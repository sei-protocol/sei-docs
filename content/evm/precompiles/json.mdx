...
<Callout type="warning">**Important:** The JSON precompile may not support dot notation for nested objects. Instead, extract the parent object and parse it manually.</Callout>
...
async function main() {
  try {
    ...
    console.log('   • JSON precompile works with direct keys, not dot notation');
    console.log('   • Extract parent objects and parse them manually for nested data');
    console.log('   • Use safe extraction functions for production code');
    console.log('   • Store decimals as integers with known precision');
    console.log('   • Handle booleans as 0/1 integers');
    console.log('   • Set appropriate gas limits based on data complexity');
    console.log('\n🔗 Explore more: https://docs.sei.io/evm/precompiles/json');
  } catch (error) {
    console.error('\n❌ Error:', error.message);
...
🎉 === JSON Precompile Demo Completed Successfully! ===
💡 Key Takeaways:
   • Use safe extraction functions for production code
   • Store decimals as integers with known precision
   • Handle booleans as 0/1 integers
   • Extract complex objects as bytes and parse manually
   • Set appropriate gas limits based on data complexity
