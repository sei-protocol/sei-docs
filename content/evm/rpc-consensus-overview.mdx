...
- **Priority-aware eviction.** On `v6.2.0+`, `GetTxPriorityHint` feeds a reservoir sampler so low-priority spam is rejected once utilisation crosses the configured threshold (`internal/mempool/mempool.go`).
