...
**Dynamic (Next.js)**, **WalletConnectKit**, and **RainbowKit** as well as web3-react hooks.
...
<details>
  ...
  // 1) Register the Sei Global Wallet under EIP‑6963 discovery
  import '@sei-js/sei-global-wallet/eip6963'
  ...
  **Explanation:**
  ...
  - **Hook‑based flow:** Wrapping in `Web3ReactProvider` and calling `activate(eip6963)` on mount (or behind a “Connect” button) gives you a familiar hook API (`useWeb3React`) and an `ethers.js` provider.  

  ———

  #### Filtering only the Sei Global Wallet

  If you need to ignore other injected wallets, you can pass a custom `getProvider` to the connector:
