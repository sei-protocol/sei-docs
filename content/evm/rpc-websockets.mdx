...
- **Heartbeat loop** - Send `eth_subscribe` keep-alives or lightweight RPC calls every ~20s to stop proxies reclaiming idle sockets.
- **Backoff strategy** - Reconnect with exponential backoff when the node drops the socket (capacity reached, idle timeout). Avoid thundering herds.
- **Replay window** - Persist the last processed block and resubscribe with a bounded `fromBlock` range to cover gaps.
