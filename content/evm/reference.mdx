...
These endpoints provide support for synthetic transactions, which represent activity from the CosmWasm virtual machine on the EVM side:

| Endpoint | Description |
|----------|-------------|
| `sei_getFilterLogs` | Like `eth_getFilterLogs` but includes synthetic logs from CosmWasm events |
| `sei_getLogs` | Like `eth_getLogs` but includes synthetic logs from CosmWasm events |
...
Returns an array of logs for a filter, including synthetic logs from CosmWasm events.
...
| `sei_queryEvents` | Enhanced event querying across EVM and CosmWasm |
...
These endpoints facilitate operations that span both the EVM and CosmWasm environments:

| Endpoint | Description |
|----------|-------------|
| `sei_callCosmos` | Allows calling CosmWasm contracts from the EVM RPC |
...
Synthetic transactions are a key concept in Sei's cross-VM architecture. They are representations of CosmWasm activity on the EVM side and are generated when:

1. CosmWasm contracts interact with pointer contracts
...
The `synthetic` field is a boolean indicator that this transaction was generated from CosmWasm activity.
...
To track token transfers across both EVM and CosmWasm environments:
...
async function trackCrossVMTransfers(tokenAddress) {
  ...
  // Get logs including synthetic logs from CosmWasm
...
async function getCleanBlock(blockNumber) {
  ...
  const receipts = await Promise.all(
    block.transactions.map(tx => 
      web3.eth.send('sei_getTransactionReceiptExcludeTraceFail', [tx.hash])
    )
  );
...
To query CosmWasm contract state from an EVM dApp:
...
- Synthetic transactions may have higher latency in appearing than native transactions
...
web3.eth.send('debug_traceTransaction', [
  txHash,
  { tracer: tracerJson }
]);
```
