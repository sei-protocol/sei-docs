...
- **Frame limits** - Keep serialized payloads below the enforced frame guard; chunk or stream large outputs.
- **Panic surfacing** - Errors thrown inside tracers appear in `error.data.trace`. Persist them for debugging.
- **Timeout & concurrency** - Respect `trace_timeout` (default 30s) and `max_concurrent_trace_calls` (default 10); excess calls queue.
