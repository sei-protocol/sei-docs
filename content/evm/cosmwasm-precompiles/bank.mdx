...
- **Payment Systems:** Create payment processors that work with multiple token denominations.
- **Portfolio Management:** Build tools to track and manage multi-asset portfolios across Cosmos and EVM.
- **Cross-Chain Trading:** Develop trading platforms that can handle diverse asset types.
...
- **Build Payment Systems** - Implement robust payment processing with proper error handling
- **Handle Multiple Denominations** - Work with various token types in a unified interface
- **Optimize Gas Usage** - Implement efficient batching and query strategies
...
contract PaymentProcessor {
    ...
    function sendSeiPaymentWithAmount(
...
const tokenDenom = 'factory/sei1.../mytoken';
const amount = '1000';
...
contract TokenRegistry {
    ...
    function registerToken(string memory denom) external {
        TokenInfo storage info = tokenRegistry[denom];
    ...
    function formatAmount(
        ...
        uint8 tokenDecimals = BANK.decimals(denom);
        // Note: Solidity doesn't have built-in decimal formatting
        // This would need additional utility functions for proper formatting
        return "Use client-side formatting";
    }
}
```
...
async function main() {
  ...
  const TokenManager = await ethers.getContractFactory('ComprehensiveTokenManager');
  ...
  const tokens = ['factory/sei1.../token1', 'ibc/ED07A3391A112B175915CD8FAF43A2DA8E4790EDE12566649D0C2F97716B8518'];
...
<Callout type="info"> Remember: Always verify token denominations and handle errors gracefully in production applications! </Callout>

### Token Denominations

- **Native SEI:** Always use `usei` as the denomination (1 SEI = 1,000,000 usei)
- **Custom Tokens:** Follow format `factory/{creator}/{subdenom}` for custom tokens
- **IBC Tokens:** Use format `ibc/{hash}` for cross-chain assets
- **Case Sensitivity:** Token denominations are case-sensitive

### Amount Precision

- **SEI Native:** 6 decimal places (1 SEI = 1,000,000 usei)
...
- **Batch Operations:** Use batch functions to reduce gas costs for multiple operations
- **Query Caching:** Cache token metadata to avoid repeated queries
- **Error Handling:** Implement proper error handling to prevent gas waste
