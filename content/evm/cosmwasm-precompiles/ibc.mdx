...
async function transferToCosmosHub() {
  ...
  const port = 'transfer';
  const channel = 'channel-0'; // Sei to Osmosis Hub channel
  const denom = 'usei';
...
contract CrossChainTransferManager {
    ...
    function transferToCosmosHub(
        ...
        bool success = IBC.transferWithDefaultTimeout(
            toAddress,
            "transfer",           // port
            "channel-0",         // channel to Osmosis Hub
            "usei",              // denomination
...
contract IBCTransferHub {
    ...
    constructor() {
        ...
        networks["osmosis"] = NetworkConfig("channel-0", true);
        networks["cosmos_hub"] = NetworkConfig("channel-1", true);
...
contract ProductionIBCManager {
    ...
    constructor() {
        ...
        configureNetwork("osmosis", "channel-0", 1000000, 100000000 * 10**6, 10000000 * 10**6);
        configureNetwork("cosmos_hub", "channel-1", 1000000, 100000000 * 10**6, 10000000 * 10**6);
    }
...
- **Osmosis:** Typically `channel-0`
- **Cosmos Hub:** Typically `channel-1`
- **Juno:** Typically `channel-2`
- **Stargaze:** Typically `channel-3`
