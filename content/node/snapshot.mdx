...
Both methods leave you with a fully‑synced node in minutes. Verify the node is catching up (`journalctl -u seid -f`) and compare the reported block height against a public RPC endpoint.
