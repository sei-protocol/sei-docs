...
<KeyValueTable
  rows={[
    ...
    ['Snapshot', '`seidadmin snapshot create --output /var/sei/snapshots/YYYYMMDD` (or provider-specific tooling).'],
    ['Peer health', 'Verify sentry/full-node connectivity; capture `~/.sei/config/peers` in case of rollback.'],
    ['Gas baseline', '`curl -s localhost:8545 ... eth_getBlockByNumber latest true` and store `gasUsed`.']
  ]}
/>
