...
- `26657`: The default port for the RPC protocol. Because this port is used for querying and sending transactions, it must be open for serving queries from `seid`. These ports are all customizable in `$HOME/.sei/config/config.toml` and `$HOME/.sei/config/app.toml` discussed in the later sections along with other fields.
