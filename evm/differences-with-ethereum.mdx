...
The wrapper-field and bloated-body rejections are consensus-level validation rules that have been enforced since v6.5.0; only the whole-block rejection on decode failure described above is new in v6.6.0. Ensure your transaction-signing tooling produces a minimal, canonical envelope with no Cosmos wrapper fields populated.
