...
Before an EVM transaction is accepted, Sei performs strict semantic validation of the transaction's fields. Transactions that are malformed at the encoding level are rejected during basic validation. These checks have been enforced since v6.5.0. If you construct and sign transactions manually, make sure they are well-formed to avoid rejection.
...
<Warning>
  Transactions with malformed signature values, access-list entries, or authorization-list entries are rejected. Ensure your signing tooling produces canonical, correctly-encoded values.
</Warning>
...
Additionally, since v6.5.0 the transaction decoder rejects "bloated" transaction bodies whose raw wire encoding is larger than the canonical re-marshaled form (for example, padded fields or oversized `Any.Value` entries). Non-canonical encodings fail with a decode error, so tooling should always emit canonical protobuf encodings.
