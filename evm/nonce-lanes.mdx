...
<Steps>
  ...
  <Step title="Choose a target network">
    Start with a local Prague fork. It carries the deployed EntryPoint bytecode from Sei Testnet but spends only Anvil funds.

    <Danger>
      Never use Anvil, Hardhat, tutorial, or shared test mnemonics on Sei Testnet or Sei Mainnet. Their addresses and keys are public, and some are already delegated to sweeper code, so a successful funding transaction can still leave a zero balance. The fork below also runs as chain `1328`, so an authorization signed against it is replayable on Sei Testnet whenever the account's nonce lines up. Regenerate every key and mnemonic before you point a fork's `.env` at a public network.
    </Danger>
