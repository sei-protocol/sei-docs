...
One funded account on Sei EVM has one sequential nonce. That is the right model for a wallet and the wrong model for a stream of independent work: a single transaction that never lands freezes every transaction signed after it. The usual fix is a fleet of hot wallets, which multiplies balances, approvals, and keys.

[sei-nonce-lanes](https://github.com/sei-protocol/sei-nonce-lanes) is a reference implementation that removes the queue instead. One funded externally owned account (EOA) keeps its address, balance, and approvals, and keeps many mutually independent operations in flight at the same time.
...
| Gas-only relayers with a write-ahead journal | Someone still has to send sequential transactions, but they hold only gas |
...
Count the nonce cost carefully. An authorization list is applied after the transaction's own nonce has been incremented, so when the authority is also the sender, the authorization must be signed over `nonce + 1`, and applying it increments the account a second time. A self-sponsored delegation therefore advances the account by two, which is the path `npm run delegate` takes. Only a delegation sponsored by a different sender costs the funded account a single increment, the one the authorization itself performs. Either way, once the designator is in place the submission path signs UserOperations only and the funded account's EVM nonce stops moving.
...
- **Write-ahead ordering.** The signed outer transaction is written to the journal before it is broadcast. Each journal snapshot is fsynced before it is renamed into place, and the directory is fsynced after it, so a record that has been written survives a lost kernel or host and not only a crashed process. On restart, the exact raw bytes are rebroadcast first, so a crash between signing and sending cannot lose or duplicate work. The journal is still one file on one disk, not a replicated database.
...
The receipt wait polls every `RECEIPT_POLLING_INTERVAL_MS`, 250 ms by default. Sei produces blocks in well under a second, and viem's 4-second default would spend most of the timeout idle between polls, then report a timeout for a bundle that had already landed and trigger a fee-bumped replacement nobody needed.

Two smaller guards keep the simulation and the journal honest. The `eth_estimateGas` call is issued from the relayer's address rather than its signing account, so the client library does not add a pending-nonce lookup of its own before the estimate. And one file lock, which identifies its holder by inode rather than by the existence of a file, keeps a second process off the same account. The process re-checks that it still holds the lock immediately before it signs and before every journal write, so a process that lost a lock race stops instead of signing.
...
Faster hardware and better RPC routing improve every submission strategy equally. What changes here is structural: it changes what a single funded address is allowed to do, and it does so with the same custody surface as a single wallet.

The honest comparison is against a fleet of hot wallets, because that is what most teams actually run. A fleet can match the width. What it cannot match is doing so from one balance, one approval set, and one key.

Be precise about what the baseline actually is. A single address was never limited to one transaction at a time: it can sign and broadcast nonces `n`, `n+1`, `n+2`, and onward and have all of them unresolved, and [the producer mempool admits them](/node/technical-reference#giga-mode-behavior-and-per-block-limits) as long as they arrive in order. The constraint is not how many it can have outstanding but that they form a single ordered queue, so one that never lands strands every later nonce behind it.
...
| Outer transactions broadcast at once | Many | Many per wallet | The bundler's policy, not yours | `RELAYER_COUNT` |
...
| Recovering the in-flight set after a crash | Re-derive it from the chain | Re-derive it per wallet | The bundler's problem, not yours | Durable journal and deterministic reconciliation |
...
Measured on Atlantic-2 on September 14, 2026, against the same venue call: one address with sequential nonces landed 1 operation per second sending serially and 7 per second pipelining one request at a time, and reached 51 per second only by sending all 1,024 transactions in one JSON-RPC batch. A fleet of 8 hot wallets also landed 51 per second. Nonce lanes landed 60 to 64 per second from one address whose EVM nonce never moved, against a block-gas ceiling of about 73 for that call shape. The method, the run shapes, and the caveats are in [Benchmarks](#benchmarks).
...
5. **Crash-safe by construction.** Signed operations and signed outer transactions are journaled and fsynced before broadcast. Replacement reuses the relayer nonce. Restart reconciliation is deterministic and refuses to create duplicate work when the state is ambiguous.
  ...
  <Step title="Choose a target network">
    ...
    <Tabs>
      <Tab title="Local Anvil fork (recommended first)">
        ...
        Those are Anvil's published credentials, which is why this `.env` belongs to the fork and nowhere else.
    ...
    <Note>
      Mutating commands refuse to write to a remote Pacific-1 RPC unless `ALLOW_MAINNET=1` is set explicitly, and they refuse any remote-chain write when `TRADER_PRIVATE_KEY` is Anvil account `0` or `RELAYER_MNEMONIC` is the Anvil mnemonic. Those guards catch an accidental `SEI_CHAIN_ID=1329` run and the two most common leaked credentials, nothing more. They do not guard the separate Forge deployment command, and they do not make the demo production-ready.
    </Note>
  </Step>
...
| Verdict | What the RPC path did | In flight per sender |
| --- | --- | --- |
| `REJECTED at admission` | Refused the gapped nonce, typically with `bad nonce` | 1; a gap cannot even be created |
| `ADMITTED then dropped` | Returned a hash, then discarded the transaction | 1 in effect, despite admission succeeding |
| `QUEUED, then included once the gap filled` | Held it and released it in nonce order | More than 1, released in order |
| `ACCEPTED and included above a gap` | Included it out of order | More than 1, not strictly ordered on this path |

The probe samples both the `latest` and `pending` nonce tags before, during, and after, and prints any rejection message verbatim, because the exact string is the finding. If the two tags already disagree before the probe starts, it warns you: either the account has work in flight or the node reports a mempool-derived pending nonce, and in both cases the gap it is about to create may not be a gap at all, so the verdict cannot be attributed to nonce ordering. Compare the result with a `submit` run, where 24 operations from one account are mutually independent and one failure strands nothing.

The verdict depends on the path. On September 14, 2026, two Atlantic-2 RPC endpoints answered differently on the same day: a dedicated provider queued the gapped transaction and released it once the gap filled, while the public endpoint returned a hash and then dropped it, so it never landed even after the gap was filled. Neither rejected it at admission. Probe the path you will actually use.
...
<AccordionGroup>
  ...
  <Accordion title="Bundle width: gas efficiency against isolation">
    ...
    A bundle does not need the sum of its operations' declared call gas, which is the arithmetic most people try first. The EntryPoint checks before each operation that enough gas remains to honor that operation's `callGasLimit`, and whatever an operation leaves unspent passes to the next one, so the outer transaction needs the gas the bundle actually consumes plus about one operation's declared limit in reserve. Declaring more than you need is still not free: v0.8 charges 10 percent of unused call gas beyond a 40,000-gas threshold, and each operation reserves prefund from the deposit against its declared limits rather than its measured cost, so an inflated limit ties up deposit that is only refunded afterwards.

    That is why `CALL_GAS_LIMIT` is unset by default. Both submit paths measure the real call gas for the chain and call shape and add 25 percent; the variable is only a floor, and it changes anything only when you set it above that measurement. Because the EntryPoint reserves each operation's declared call gas before running it, a floor above the measurement costs block gas, and therefore operations per block, without changing the gas actually used.

    As a reference point, on Atlantic-2 on September 3, 2026, with `CALL_GAS_LIMIT=500000`, which was the default at the time, the real-swap path sustained 77 operations per bundle; 78 no longer fit the 12,500,000 block gas limit in effect that day and failed safely during simulation. Width 76 produced the best observed submission rate for that call shape, 47.7 landed swaps per second. Treat these as measurements for one call shape, one configuration, and one network at one point in time, not as protocol limits.

    The mock venue is a heavier call. On September 14, 2026, with `CALL_GAS_LIMIT` unset, each `place` operation used about 331,000 gas of outer transaction gas, so 36 operations was the widest bundle that fit and 37 failed safely in simulation. Widths 8, 16, and 36 packed 4, 2, and 1 bundles into a 12,500,000-gas block; width 9, which should fit 4, landed 3. Wide bundles also queue behind each other, because only one 11,700,000-gas bundle fits a block. With more than about 8 relayers at width 32 or 36, receipt waits exceed the default `BUNDLE_RECEIPT_TIMEOUT_MS`; raise the timeout for those shapes rather than paying for fee-bumped replacements of bundles that will land anyway.
  </Accordion>
  <Accordion title="Relayer count: concurrent outer transaction streams">
    ...
    One relayer's bundle cycle is five sequential RPC round trips (gas estimate, fee estimate, block number, broadcast, receipt poll) plus inclusion, about 1.5 seconds on a 120 ms endpoint. Below roughly 120 operations in flight the pool is the bottleneck and throughput scales with `RELAYER_COUNT`; above that, block gas is. In the September 14, 2026 measurements, 4 relayers landed 2.7, 8.9, 15.3, 24.3, 45.4, and 50.5 operations per second at widths 1, 4, 8, 16, 32, and 36, and width 4 landed 2.2, 8.9, 15.4, 27.9, and 47.2 per second with 1, 4, 8, 16, and 32 relayers.
  </Accordion>
</AccordionGroup>
...
Every row below was measured on Atlantic-2 on September 14, 2026. Conditions: block gas limit 12,500,000; base fee 50 gwei plus a 1 gwei tip; about 2.0 blocks per second under load; 120 ms warm request latency to the configured endpoint. Each direct `place` transaction used 328,425 gas and each lane operation about 331,000 gas of outer transaction gas, so the block admits 36 to 38 operations however they are submitted: roughly 73 landed operations per second for this call shape. Chain-side rates divide landed operations by the block-timestamp span, which Sei stamps in whole seconds. Client-side rates divide by wall time from first broadcast to last receipt.
...
| Fleet of 4 wallets × 100 tx | 26.7 | 24.8 | |
...
- **RPC paths differ.** `npm run baseline` gave [two different verdicts](#optional-see-the-baseline-you-are-escaping) for two Atlantic-2 endpoints on the same day.
...
<AccordionGroup>
  ...
  <Accordion title="Journal lock is owned by another process">
    ...
    `could not acquire the lock … within 2000ms` is different. The lock file exists but its contents could not be read, which is what a live holder can look like while it is still writing the file. Removing such a lock would evict a running process, so the next run waits for it instead. If the message persists across runs, no process owns the file and you can delete it by hand.
  </Accordion>
