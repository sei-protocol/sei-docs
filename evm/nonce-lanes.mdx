...
<Steps>
  ...
  <Step title="Choose a target network">
    ...
    <Danger>
      Never use Anvil, Hardhat, tutorial, or shared test mnemonics on Sei testnet or mainnet. Their addresses and keys are public. All ten published Anvil accounts already delegate to sweeper code on both networks and sit at a zero balance, so a successful funding transaction can still leave you with nothing. A fork inherits that state, which is why the fork below needs a fresh mnemonic too. The fork also runs as chain `1328`, so an authorization signed against it is replayable on Sei testnet itself whenever the account's nonce lines up. Regenerate every key and mnemonic before you point a fork's `.env` at a public network.
    </Danger>
    ...
    <Tabs>
      <Tab title="Local Anvil fork (recommended first)">
        ...
        Anvil funds the first ten accounts of whatever mnemonic you hand it, so a fresh phrase gives you ten funded addresses with no inherited code. Anvil's own accounts do not work here. A fork overrides an account's balance but keeps the code it inherits, and every published Anvil address is already delegated on both Sei networks, so those addresses arrive on the fork with a delegation designator in place. Every mutating command refuses a relayer that has code.
  ...
  <Step title="Check the preflight">
    ...
    It prints the chain ID, confirms code at the EntryPoint address `0x4337084D9E255Ff0702461CF8895CE9E3b5Ff108`, and reports the account's current delegation, balances, its EntryPoint deposit, each relayer's confirmed nonce and gas balance, the lane sequences, and the venue state.

    Against Sei testnet or mainnet the EntryPoint line always reads `present`, because the singleton is deployed on both. The check earns its place on a local fork or against a misconfigured endpoint, where it tells you that you are not on Sei at all. The relayer lines are worth a second look: any `HAS CODE/DELEGATION` marker means the mutating commands will refuse that relayer.
  </Step>
...
<AccordionGroup>
  ...
  <Accordion title="EntryPoint v0.8 … MISSING">
    ...
    Confirm the deployment yourself instead of diffing bytecode:
  ...
  <Accordion title="Refuses a relayer that already has code or a delegation">
    Relayers must be plain EOAs, and every mutating command checks that before it signs. The usual cause is a mnemonic whose addresses are already delegated on Sei. That is true of every published Anvil account on both networks, and a fork of either inherits it. Generate a fresh mnemonic with `cast wallet new-mnemonic`, point `RELAYER_MNEMONIC` at it, and fund the new relayers rather than trying to undelegate the old ones.
  </Accordion>
