...
import { SequentialNonceQueue, NonceLanes, NonceLaneDelegation, NonceLanePipeline, NonceLaneBundleLifecycle, NonceLaneFailureIsolation, NonceLaneThroughputComparison, NonceLaneBlockGasUtilization, NonceLaneScalingSeries } from '/snippets/nonce-lane-diagrams.jsx';
...
The canonical EntryPoint v0.8 singleton is already deployed at `0x4337084D9E255Ff0702461CF8895CE9E3b5Ff108` on both Sei mainnet (Pacific-1, chain ID `1329`) and Sei testnet (Atlantic-2, chain ID `1328`), so nothing here asks you to deploy one. It stores a UserOperation nonce as a 192-bit key plus a 64-bit sequence:
...
Measured on Sei testnet against the same venue call, one address with sequential nonces landed 1 operation per second sending serially and 7 per second pipelining one request at a time, and reached 51 per second only by sending all 1,024 transactions in one JSON-RPC batch. A fleet of 8 hot wallets also landed 51 per second. Nonce lanes landed 60 to 64 per second from one address whose EVM nonce never moved, against a block-gas ceiling of about 73 for that call shape. The method, the run shapes, and the caveats are in [Benchmarks](#benchmarks).
...
- For the Sei testnet path: a fresh throwaway key funded from the [Sei faucet](/learn/faucet)
...
<Steps>
  ...
  <Step title="Choose a target network">
    ...
    <Danger>
      Never use Anvil, Hardhat, tutorial, or shared test mnemonics on Sei testnet or mainnet. Their addresses and keys are public, so anyone can move funds sent to them. An address derived from a published mnemonic may also carry an existing EIP-7702 delegation, and a fork inherits that code. The fork below therefore needs a fresh mnemonic too. It also runs as chain `1328`, so an authorization signed against it is replayable on Sei testnet itself whenever the account's nonce lines up. Regenerate every key and mnemonic before you point a fork's `.env` at a public network.
    </Danger>

    <Tabs>
      <Tab title="Local Anvil fork (recommended first)">
        EIP-7702 requires a Prague-capable node. Generate a mnemonic for the fork to use, and do not fall back to Anvil's default one:
        ...
        Anvil funds the first ten accounts of whatever mnemonic you hand it. Using a fresh phrase avoids Anvil's published addresses. Do not use the default mnemonic here: a fork keeps any account code already attached to those public addresses, and every mutating command refuses a relayer that has code.
        ...
        Set `TRADER_PRIVATE_KEY` to account `0`'s private key from the Anvil startup output, and `RELAYER_MNEMONIC` to the phrase you just generated. Starting relayers at index `1` keeps the funded account and relayer identities distinct; the application rejects overlapping identities.

        Anvil prints that key in the clear and it lands in `.env`, so treat the phrase as throwaway and keep this `.env` pointed at `127.0.0.1`.

        Deploy the demo contracts from the repository root. Anvil unlocks its own accounts, so pass account `0`'s address from its startup output as the sender:
  ...
  <Step title="Check the preflight">
    ...
    It prints the chain ID and whether there is code at the EntryPoint address `0x4337084D9E255Ff0702461CF8895CE9E3b5Ff108`. It also reports the account's current delegation, balances, its EntryPoint deposit, each relayer's confirmed nonce and gas balance, the lane sequences, and the venue state.

    Against current Sei testnet or mainnet state, the EntryPoint line reads `present` because the singleton is deployed on both. On a local fork or against a misconfigured endpoint, the check catches missing fork state or the wrong network. The relayer lines are worth a second look: any `HAS CODE/DELEGATION` marker means the mutating commands will refuse that relayer.
  </Step>
...
The repository includes a real-target path that routes tiny native SEI and native USDC swaps through the documented DragonSwap V1 deployment on Sei testnet. It is hard-blocked on every other chain. Get testnet USDC from the [Circle faucet](https://faucet.circle.com/), then:
...
<AccordionGroup>
  ...
  <Accordion title="Relayer count: concurrent outer transaction streams">
    ...
    One relayer's bundle cycle is five sequential RPC round trips (gas estimate, fee estimate, block number, broadcast, receipt poll) plus inclusion, about 1.5 seconds on a 120 ms endpoint. Below roughly 120 operations in flight the pool is the bottleneck and throughput scales with `RELAYER_COUNT`; above that, block gas is.

    In the [benchmark session](#benchmarks), the bundle-width sweep with 4 relayers produced client-side rates of 2.7, 8.9, 15.3, 24.3, 45.4, and 50.5 operations per second at widths 1, 4, 8, 16, 32, and 36. Its chain-side rates were 3.2, 9.8, 17.1, 28.4, 51.2, and 56.9. At width 4, the relayer-count sweep produced client-side rates of 2.2, 8.9, 15.4, 27.9, and 47.2 operations per second with 1, 4, 8, 16, and 32 relayers. Its chain-side rates were 2.7, 9.8, 17.1, 32.0, and 51.2.

    <NonceLaneScalingSeries />
  </Accordion>
</AccordionGroup>
...
| --- | --- | --- |
| `SEI_CHAIN_ID` | `1328` | `1328` (Sei testnet) or `1329` (Sei mainnet) |
| `SEI_RPC_URL` | viem chain default | HTTP endpoint; its reported chain ID must match before any write |
| `ALLOW_MAINNET` | `0` | Must be `1` for writes to a remote Sei mainnet RPC |
...
Every row below comes from one measurement session on Sei testnet. Conditions during the session: block gas limit 12,500,000; base fee 50 gwei plus a 1 gwei tip; about 2.0 blocks per second under load; 120 ms warm request latency to the configured endpoint. Each direct `place` transaction used 328,425 gas and each lane operation about 331,000 gas of outer transaction gas, so the block admits 36 to 38 operations however they are submitted: roughly 73 landed operations per second for this call shape. Chain-side rates divide landed operations by the block-timestamp span, which Sei stamps in whole seconds. Client-side rates divide by wall time from first broadcast to last receipt.

<NonceLaneThroughputComparison />

| Submission | Landed ops/s, chain-side | Client-side | Block gas: average / fullest / blocks ≥ 90% |
| --- | --- | --- | --- |
...
| Tutorial lanes, 4 relayers × 4 per bundle (24 ops) | 8.0 | 5.7 | 10.6% / 31.1% / 0 of 7 |
| Fleet of 4 wallets × 100 tx | 26.7 | 24.8 | not recorded |
| Fleet of 8 wallets × 64 tx | 51.2 | 49.7 | 54.4% / 66.7% / 0 of 25 |
| One EOA, 1,024 tx in one JSON-RPC batch | 51.2 | 47.3 | 68.1% / 97.2% / 24 of 40 |
| Lanes, 32 relayers × 8 per bundle (1,024 ops) | 60.2 | 55.3 | 82.5% / 94.6% / 1 of 34 |
| Lanes, 16 relayers × 16 per bundle (1,024 ops) | 64.0 | 54.6 | 81.1% / 91.4% / 1 of 34 |
| Lanes, 16 relayers × 36 per bundle (1,024 ops) | 64.0 | 55.1 | 88.4% / 98.1% / 28 of 31 |

<NonceLaneBlockGasUtilization />
...
- RPC paths differ. `npm run baseline` gave [two different verdicts](#optional-see-the-baseline-you-are-escaping) for two Sei testnet endpoints in the same session.
...
<AccordionGroup>
  ...
  <Accordion title="EntryPoint v0.8 … MISSING">
    The canonical v0.8 singleton is deployed at `0x4337084D9E255Ff0702461CF8895CE9E3b5Ff108` on both Sei mainnet and Sei testnet. A `MISSING` line against current state usually means the configured RPC is not serving Sei. Check `SEI_RPC_URL`, and check that a local Anvil was started with `--fork-url` against a Sei endpoint rather than as a bare chain. `submit` fails closed on the same condition with `No EntryPoint at … on this chain`, before it signs anything.

    Do not deploy your own EntryPoint to work around it. A second EntryPoint lives at a different address, so it has a different EIP-712 domain and shares no lane sequences or deposits with the one every other integration on Sei uses.

    Confirm the deployment against the same endpoint. Export `SEI_RPC_URL` from your `.env`, then run:
    ...
    It answers `ERC4337`, version `1`, its own address, and the chain ID you queried. Comparing bytecode against another chain's deployment is the wrong test: v0.8 caches that domain separator in an immutable, so the runtime code at the canonical address differs by that one word, and by the chain ID word, on every chain it is deployed to.
  </Accordion>
