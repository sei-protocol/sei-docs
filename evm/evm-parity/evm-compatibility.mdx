...
| Feature | Notes |
| --- | --- |
| Sei precompiles (staking, governance, distribution, P256, JSON, and the CosmWasm bridge) | EVM contracts at deterministic addresses. ABIs and contract addresses are exported from `@sei-js/precompiles` for use with any standard EVM library. The retired Oracle and disabled IBC precompiles are not exported. |
