...
2. **Funding Requirements**: On Sei testnet, all transactions are
   automatically sponsored, meaning you don't need to deposit funds to cover
   transaction fees. However, on Sei mainnet, the Paymaster (configurable
...
login or a Web3 login and sends a gasless transaction of 0.001 SEI via the
`ethers` provider.
