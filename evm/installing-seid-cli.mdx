...
<Note>The `compact`, `latest_version`, and `prune` maintenance commands were removed in v6.6.0. Historical-data retention is now managed through SeiDB configuration (`min-retain-blocks`, `ss-keep-recent`) and the `seidb` tool.</Note>
