...
- [Go 1.25.6 or later](https://go.dev/doc/install) is required only when building from source. Check the selected release's [`go.mod`](https://github.com/sei-protocol/sei-chain/blob/main/go.mod) for its exact requirement. Prebuilt binaries have no prerequisites.
