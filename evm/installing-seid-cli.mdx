...
Available Commands:
  ...
  log                      Runtime log management
  migrate                  Migrate genesis to a specified target version
  query                    Querying subcommands
  rollback                 rollback cosmos-sdk and tendermint state by one height
  snapshot                 Create a snapshot using the snapshot manager
...
<Note>The `compact`, `latest_version`, and `prune` maintenance commands were removed in v6.6.0; the listing above also includes the `log` and `snapshot` commands, which were previously missing from this page. To read the latest memIAVL version (the former `latest_version`), use `seidb memiavl-latest-version`. Historical-data retention is managed through `min-retain-blocks` in the base `app.toml` and `ss-keep-recent` under `[state-store]`.</Note>
