...
```solidity
function getSEIPrice() external view returns (int224 value) {
    (value, ) = readDataFeed();
    // `value` has 18 decimals, e.g. 300000000000000000 = $0.30.
    // Divide by 1e18 off-chain (or scale on-chain) to get the dollar price.
}
```
