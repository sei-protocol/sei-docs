...
<Info>
  Per [Proposal 115](https://www.mintscan.io/sei/proposals/115), no new CosmWasm contracts can be deployed on Sei. The CW20 / CW721 / CW1155 pointer-creation paths on this precompile only apply to **already-deployed** CosmWasm contracts. Native (Bank Module) pointers via `addNativePointer` continue to work normally for any registered bank-module asset (excluding `usei`). For new tokens or NFTs, deploy ERC20 / ERC721 / ERC1155 contracts directly on the EVM.
</Info>
