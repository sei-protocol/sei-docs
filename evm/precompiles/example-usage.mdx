...
| Address | `0x1004` | Convert and associate EVM and Sei addresses |
...
| Pointer view | `0x100A` | Look up pointer contract addresses |
| Pointer | `0x100B` | Register pointer contracts |
| Solo | `0x100C` | Access Solo migration functions |
...
<Note>Import the raw `*_PRECOMPILE_ABI` constants for both Viem and Ethers.js. The package does not expose the old `VIEM_*_PRECOMPILE_ABI` aliases.</Note>
...
const tx = await distribution.withdrawDelegationRewards('seivaloper1...');
await tx.wait();
```
