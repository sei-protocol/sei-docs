...
Import precompile addresses and raw `*_PRECOMPILE_ABI` constants from the package root, `@sei-js/precompiles/precompiles`, or `@sei-js/precompiles/viem`. Ethers factories live on `@sei-js/precompiles/ethers`. The package re-exports Viem's `sei` and `seiTestnet` chain definitions from the root and `viem` entrypoints.
