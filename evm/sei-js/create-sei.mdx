...
Open `http://localhost:3000`. The generated dApp connects to Sei Mainnet when `NEXT_PUBLIC_CHAIN` is unset. Set `NEXT_PUBLIC_CHAIN=testnet` in `.env.local` to use Sei Testnet. The template's `.env.example` already sets that value, so copying it to `.env.local` selects testnet.
</Step>
...
- Sei Mainnet and Sei Testnet configuration via `NEXT_PUBLIC_CHAIN`
