...
| `trace_bake_use_snapshot` | `false` | Uses in-memory memiavl snapshots as the state backend for trace baking when the store backend supports snapshots. |
| `trace_bake_snapshot_window` | `64` | Number of recent memiavl snapshots to retain for trace baking. |
