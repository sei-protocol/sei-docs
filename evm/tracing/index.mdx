...
<Info>Earlier behavior excluded any receipt with `Status == 0`, which over-filtered reverts and OOG failures that actually produced traces. The current logic excludes only ante-error and synthetic txs.</Info>
