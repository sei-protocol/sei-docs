# Gasless transactions with Particle Connect

[Particle Network](https://particle.network) simplifies onboarding into smart accounts using its Wallet Abstraction technology. It offers social logins and wallet connection kits fully compatible with Sei, enabling quick access to Web3 applications.

The Particle Network user flow begins with social login, either through custom authentication or predefined options offered by the platform. This process generates an EOA using MPC-TSS, which then acts as the Signer for a smart account. This can include implementations like `SimpleAccount` or Biconomy (V1/V2), depending on the application's requirements.

This guide provides a high-level overview of the steps required to build a demo application on Sei using the [Particle Connect SDK](https://developers.particle.network/api-reference/connect/desktop/web), Particle's leading onboarding solution. The SDK simplifies user onboarding by offering a unified interface that supports both social login and Web3 login options in a single modal.
...
In this tutorial, we’ll demonstrate how to create a wallet on Sei using social logins within a [Next.js](https://nextjs.org/docs/getting-started/installation) app powered by TypeScript and styled with Tailwind CSS.

The Particle Connect SDK streamlines wallet creation, user authentication, and blockchain interactions into a unified interface. It supports social logins and traditional Web3 wallets, making it an all-in-one solution for seamless user onboarding.
...
In this example, the application creates a smart account on Sei using a social login or a Web3 login and sends a gasless transaction of 0.01 ETH via the `ethers` provider.
