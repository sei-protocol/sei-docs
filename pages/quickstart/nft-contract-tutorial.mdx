...
2. Deploy your contract to a local testnet (e.g., using Anvil, Foundry's local Ethereum node):
   ...
   anvil -a 1
   ```
    `-a` flag specifies the number of test accounts to create. This command will start a local Ethereum node and
   ...
   ```
    Use `--rpc-url` flag to specify the URL of the Ethereum node. In this case, it's the URL of the local Anvil node.
    Use `--private-key` flag to specify the private key of the account that will deploy the contract.
    Anvil generated test account's private key is used in this case.

3. For deploying to the Sei devnet, run `forge create` command with the `--rpc-url` flag to set to Sei EVM devnet endpoint
    and `--private-key` flag set to the private key of your account on the Sei devnet.
