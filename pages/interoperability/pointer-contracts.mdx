...
import PointerContractsWith from "../../public/assets/pointer-contracts-with.png";
...
Pointer Contracts enable tokens to be use interoperably in both EVM and CosmWasm environments.

## Why Pointer Contracts

<ImageWithCaption img={PointerContractsWithout} alt="Without Pointer Contracts" />
Wallets and clients typically only support a single execution environment. For example, EVM Native Wallets allow users to transfer and see balances of ERC-20 coins and ERC-721 NFTs.
...
<ImageWithCaption img={PointerContractsWith} alt="Without Pointer Contracts" caption="With pointer contracts, all tokens can be accessed by wallets of either environment"/>
