...
3. For deploying to a public testnet or mainnet, configure your `foundry.toml` with the appropriate RPC URL and deployer account details. Then, run `forge create` with the `--rpc-url` and `--private-key` flags.

<Callout type="info" emoji="ℹ️">
  Make sure to have ETH in your deployer account to cover the gas fees for
  contract deployment.
</Callout>
