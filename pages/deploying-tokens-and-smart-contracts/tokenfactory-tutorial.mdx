...
```bash copy
seid tx tokenfactory create-denom $SUBDENOM --from=$ACCOUNT --chain-id=atlantic-2 --node=https://rpc.atlantic-2.seinetwork.io/ --broadcast-mode=block --fee=20000usei
```
...
- `--fee=20000usei`: This is used to specify the transaction fee.
...
```bash copy
seid tx tokenfactory mint $AMOUNT --from=$ACCOUNT --chain-id=atlantic-2 --node=https://rpc.atlantic-2.seinetwork.io/ --broadcast-mode=block --fee=20000usei
```
...
```bash copy
seid tx tokenfactory burn $AMOUNT --from=$ACCOUNT --chain-id=atlantic-2 --node=https://rpc.atlantic-2.seinetwork.io/ --broadcast-mode=block --fee=20000usei
```
...
For more advanced features and details, please refer to the [Token Factory module](https://github.com/sei-protocol/sei-chain/tree/main/x/tokenfactory).
