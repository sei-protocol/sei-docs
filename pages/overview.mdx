...
The EVM (Ethereum Virtual Machine) is used to process transactions in blockhains such as Ethereum. Most crypto native developers are extremely familiar with the EVM.
...
- **Interoperable EVM**: This allows existing developers in the Ethereum ecosystem to deploy their applications, tooling and infrastructure to Sei with no changes, while benefiting from the 500x performance improvements offered by Sei.
