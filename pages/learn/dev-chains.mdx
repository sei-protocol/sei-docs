...
<Blockquote mt="lg">All chain updates are first deployed to the devnet to ensure stability. They are then moved to the testnet for further testing and validation. Finally, the updates are released to the mainnet. This process allows developers to test their applications thoroughly and raise any concerns regarding the upgrades before they go live on the mainnet.</Blockquote>

## Mainnet

The {<Code>{CHAIN_IDS.mainnet}</Code>} chain is the mainnet of the Sei
...
The {<Code>{CHAIN_IDS.testnet}</Code>} chain is the testnet of the Sei
...
The {<Code>{CHAIN_IDS.devnet}</Code>} chain is the devnet of the Sei blockchain.
