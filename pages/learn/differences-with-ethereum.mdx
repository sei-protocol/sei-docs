...
Sei supports all transaction types. However for a legacy (non EIP1559) type transaction, you must specify a base fee of 1gwei.
In addition to this, excess "gas wanted/gas limit" beyond the actual "gas used" may not be refunded in full or in part.

Current EIP1559 parameters can be fetched from the RPC:

```sh
curl -s "https://rpc.pacific-1.sei.io/abci_query?path=%22/custom/params/subspaces/evm%22"
...
Sei has a gas limit of 10M on {<Code>{CHAIN_IDS.mainnet}</Code>}, compared to Ethereum’s 30M.
