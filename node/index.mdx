...
Comprehensive guides for running, maintaining, and optimizing Sei network nodes. Whether you're setting up a validator, an RPC node, or an archive node, you'll find detailed instructions and best practices to ensure optimal performance and security.
...
<Tabs>
  ...
  <Tab title="Docker">
    ...
    <Warning>
      ...
      - `mock_chain_validation-*` — swallows most halting consensus validation failures (every swallow-eligible check except `ErrLastCommitVerify`), computing each check authentically but continuing instead of halting. Built with both the `mock_balances` and `mock_chain_validation` Go build tags. Nodes running these non-default consensus policies emit a `sei_unsafe_validation_skipped_total` counter (attribute `validation_error`) each time a failure is swallowed; on production policies it never increments.
...
# Zero (or unset) disables TTL purging entirely — see the mempool TTL
# note in Node Operators before changing these.
ttl-duration = "5s"
...
<CardGroup cols={3}>
  ...
  <Card horizontal title="Giga SS Store Migration" icon="database" href="/node/giga-storage-migration">
    Split EVM historical state into dedicated state-store databases on RPC nodes. Requires a full state sync.
  </Card>
