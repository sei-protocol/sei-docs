...
### Giga SS Store startup refusal

`seid` refuses to start if you set `evm-ss-split = true` against a data
directory that already has Cosmos SS history but no populated EVM SS
database. The composite store will not fall back to Cosmos SS for EVM
reads.

Do not flip the flag on a live data directory. Either complete the
[Giga SS Store migration](/node/giga-storage-migration) (state sync into
the new layout) or set `evm-ss-split = false` and restart. If
`data/evm_ss/` (or your `evm-ss-db-directory`) is leftover from a failed
attempt, remove it before you state-sync.

This flow is RPC-only. Validator and archive nodes should keep
`evm-ss-split = false`.
...
<Note>As with FlatKV, the memIAVL store has two possible locations: nodes created before the layout change keep the legacy `$HOME/.sei/data/committer.db` shown above (it takes precedence when present), while new nodes use `$HOME/.sei/data/state_commit/memiavl`. Point `-d` at whichever exists on your node.</Note>
...
For targeted debugging, the command also supports inspecting a single normalized bucket instead of printing the global digest. The full flag reference (inspect mode, sharding, `--find-hash`) lives in the [seidb tooling section of the technical reference](/node/technical-reference#comparing-evm-state-across-backends). For example, to list the first 50 `account` rows with version metadata, or to shard the `storage` bucket under a key prefix by the next 2 bytes:
