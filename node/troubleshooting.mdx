...
1. **Update peer configurations** with current node IDs (the embed below shuffles fresh `persistent_peers` for `config.toml`; click **Shuffle** to redraw, **Copy** to grab the line):
