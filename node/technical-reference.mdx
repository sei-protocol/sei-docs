...
When a non-default policy swallows a failure, it increments the `sei_unsafe_validation_skipped_total` counter (labeled with the failure `kind`) instead of halting. This metric is always zero in production builds.
