...
|---|---|---|
| **SC** (State Commit, app hash) | `memiavl` | FlatKV (optional, off by default) |
...
- If the node still uses RocksDB for the state store, you can move to PebbleDB
  in this same state sync. `ss-backend` drives both the Cosmos SS MVCC
  database and the EVM SS sub-databases. Set `ss-backend = "pebbledb"` before
  you start. Do not switch to RocksDB.
...
<Warning>Keep `ss-backend = "pebbledb"` during this migration. RocksDB support for the state store will be removed. No target release has been published. If the node already uses RocksDB, follow [Move off RocksDB](/node/node-operators#move-off-rocksdb) and set `evm-ss-split = true` before the same state sync.</Warning>

</Step>
<Step title="State sync into the new layout">
...
# 3. Re-init, then re-apply config.toml and app.toml (set Step 1 values again)
...
<Info>
`seid init` rewrites `app.toml` and `config.toml`. After init, set `evm-ss-split = true` and `ss-backend = "pebbledb"` again, then follow the [Statesync guide](/node/statesync) to fill in trust height, trust hash, and persistent peers.
</Info>

<Warning>Keep `priv_validator_key.json` in safe storage before deleting it from the config directory. Loss of this key is unrecoverable for a validator. This migration is RPC-only, so that key should not be in play, but do not follow this checklist on a validator.</Warning>
...
Check the startup logs for all three of these lines:
...
Then confirm EVM RPC with `debug_traceBlockByNumber`. That call forces the
node to read EVM state out of the new EVM SS backend:
...
- SC data (`memiavl`, plus FlatKV if you have opted into it) is untouched by
  this migration.
