...
`flatkv_only` is only valid for a node that boots or state-syncs directly into
the fully-migrated shape — it is not a flip target for an `evm_migrated` node,
which still holds `bank/` and every other non-EVM module in memiavl. Use
`flatkv_only` when you want a node to come up already in the post-migration
steady state rather than draining keys out of memiavl at runtime.
