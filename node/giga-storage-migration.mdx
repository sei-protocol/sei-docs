...
`sc-enable-lattice-hash` are no longer emitted in the generated `app.toml`
template. They still exist on the underlying `[state-commit]` config and
default to `cosmos_only` / `cosmos_only` / `false`, but you no longer need to
set them for this migration.

If you do configure the SC layer to use `split_write` mode
(`sc-write-mode = "split_write"`), the lattice hash must be enabled
(`sc-enable-lattice-hash = true`). State-commit config validation now rejects
