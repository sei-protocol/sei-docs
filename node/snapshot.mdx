...
<Info>
Giga SS Store migration is a [state sync](/node/statesync) flow, not a filesystem snapshot restore. Set `evm-ss-split = true` and follow the [Giga SS Store Migration Guide](/node/giga-storage-migration). A snapshot taken from a node that has not split EVM SS will fail the startup safety checks if you enable the split against it.
</Info>
