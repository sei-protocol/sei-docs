...
sidebarTitle: 'Seictl Setup Helper'
...
<Note>
  This is newer than the Go version needed to build `seid` (1.25.6). If you installed Go by
  following the [node setup guide](/node), your toolchain is a minor version too old and
  `go build` will fail — upgrade to Go 1.26.0 or later, or use a prebuilt binary above.
</Note>
