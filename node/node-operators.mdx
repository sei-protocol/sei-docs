...
| `max_gas_per_block` | Maximum gas per produced block. Must be `> 0`. |
...
| `mempool_size` | Retained for compatibility. Must be `> 0`. Note that the Autobahn producer no longer maintains a separate producer mempool channel — blocks are now built by reaping transactions directly from the shared node `TxMempool` (configured under the `[mempool]` section of `config.toml`). |
