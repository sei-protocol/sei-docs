...
# Route EVM historical state to a dedicated SS backend. Leave false unless
# you have completed the Giga SS Store migration (RPC nodes only).
evm-ss-split = false
...
error `unsupported receipt-store backend "parquet"; supported: pebbledb`. Enabling Giga
...
<Info>
RPC nodes can split EVM historical state into a dedicated database with
`evm-ss-split = true`. That requires a full state sync. Follow the
[Giga SS Store Migration Guide](/node/giga-storage-migration). Validator and
archive nodes are not supported by that flow yet.
</Info>
...
<Note>
Two behaviors are not spelled out in the generated comments above.
`min-retain-blocks` also drives receipt store retention: the receipt store's
`KeepRecent` is derived from `min-retain-blocks`, and `0` means keep
everything. When `evm-ss-db-directory` is unset, nodes with an existing
...
<Note>
Out-of-process Application Blockchain Interface (ABCI) support has been
removed: the node runs only with Tendermint in-process, so the legacy
`proxy-app` and `abci` base config fields no longer appear in the generated
file and have no effect. When upgrading, remove any `proxy-app` and `abci`
lines from your existing `config.toml`.
</Note>

<Note>
The `[p2p]` section also accepts `max-outbound-connections`, which `seid init`
does not emit. Inbound and outbound connections are managed in separate pools:
this field caps the outbound pool, and the inbound pool is derived as
`max-connections - max-outbound-connections`. Persistent and unconditional
peers bypass both limits. When left unset, it defaults to 20, or to half of
`max-connections` (rounded up) when `max-connections` is below 40. When set
explicitly, the effective value is
`min(max-connections, max-outbound-connections)`.
</Note>

<Note>
The Tendermint mempool was substantially rewritten. Transaction admission,
ordering, eviction, and gossip are now driven by a unified transaction store
with inclusion-ordering, soft/hard size limits, and cache-based deduplication,
so node operators may observe different ordering, eviction, and gossip
behavior after upgrading. A zero `ttl-duration` or `ttl-num-blocks` disables
that form of TTL purging entirely. Expired PENDING transactions are always
pruned; expired READY transactions are pruned when
`remove-expired-txs-from-queue` is `true` (the default). The
`pending-ttl-duration` and `pending-ttl-num-blocks` fields are deprecated and
have no effect — pending transactions are no longer purged based on their own
TTL.
</Note>

<Note>
The `[consensus]` section also accepts `unsafe-overrides-enabled` (default
`false`), which `seid init` does not emit. The `unsafe-*-override` timeout
fields take effect only when `unsafe-overrides-enabled` is `true`, or when the
onchain timeout params still equal the legacy pacific-1 values (a
backward-compatibility carve-out during migration). Otherwise the overrides
are ignored and the onchain (or default) timeout params are used.
</Note>
...
1. **State Commit (SC)** — the active chain state used for transaction
   ...
   `memiavl_only` — FlatKV is only used when `sc-write-mode` is set to a
   migration mode or `flatkv_only`.
2. **State Store (SS)** — versioned raw key/value pairs used for historical
   ...
   **PebbleDB**. Do not use RocksDB for new nodes. If you already use RocksDB,
   ...
   RPC nodes can optionally split EVM historical state into a dedicated EVM SS
   database (`evm-ss-split = true`). That change is per-node, does not affect
   the app hash, and requires a full state sync. See the
   [Giga SS Store Migration Guide](/node/giga-storage-migration). Validator
   and archive nodes are not supported by that flow yet.

The legacy IAVL backend has been fully removed. Setting `sc-enable = false`
...
```toml
[state-commit]
# SeiDB state-commit is mandatory; setting this to false panics at startup.
sc-enable = true
...
# all_migrated_but_bank, migrate_bank, flatkv_only, test_only_dual_write.
# Note: sc-read-mode and sc-enable-lattice-hash, which appeared in earlier
# versions of this page, were never seid config keys; if present in an
# existing app.toml they are silently ignored and can be removed.
sc-write-mode = "memiavl_only"

# How many EVM keys the in-flight migration (sc-write-mode = migrate_evm /
# migrate_bank / migrate_all_but_bank) drains from memiavl into FlatKV per
# block. Must be > 0; ignored when not in a migration mode.
sc-keys-to-migrate-per-block = 1024

[state-commit.flatkv]
# FlatKV is the EVM-optimized commit store (PebbleDB). With the default
# sc-write-mode = "memiavl_only" it is not used and these settings have
# no effect. WAL crash recovery is idempotent, so fsync stays off by
# default.
fsync = false
async-write-buffer = 0
snapshot-interval = 10000
...
# Optional EVM SS routing — when true, EVM data is routed exclusively to a
# dedicated EVM SS backend (no fallback between backends). Leave false
# unless you have completed the Giga SS Store migration.
evm-ss-split = false
evm-ss-separate-dbs = false

[receipt-store]
# pebbledb is the only supported backend (parquet was removed in v6.6.0).
# Receipts are pruned together with blocks according to min-retain-blocks above.
rs-backend = "pebbledb"
async-write-buffer = 100
prune-interval-seconds = 600
```
...
3. Set `ss-backend = "pebbledb"`, then rebuild through
   ...
   RPC nodes that are also enabling Giga SS Store should set
   `evm-ss-split = true` before that same state sync. One rebuild can
   populate both PebbleDB and the split EVM SS layout. See the
   [Giga SS Store Migration Guide](/node/giga-storage-migration).
...
```toml
[state-store]
# Route EVM data to a dedicated EVM SS backend (Giga SS Store migration).
evm-ss-split = true
```

The SC layer has its own, separate migration path (`sc-write-mode` and the
FlatKV drain), with different coordination requirements and its own warnings —
do not combine the two flips in one restart. See the
[FlatKV EVM SC migration flow](/node/giga-storage-migration#flatkv-evm-sc-migration-flow)
before touching `sc-write-mode`.
...
newly generated configurations and configurations that omit these fields
default to the Giga execution path with OCC enabled. Set both fields explicitly
to `false` to opt out. Follow the release-specific operator instructions before
changing either setting:

```toml
[giga_executor]
# Use the Giga execution path.
# Default: true.
enabled = true
