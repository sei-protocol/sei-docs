...
Each `node-dir` argument must contain three files describing that committee
member:
...
in the same directory as the key file. You only need to supply
`autobahn_address.txt` yourself. Point `autobahn-config-file` at the generated
file to enable Autobahn.
