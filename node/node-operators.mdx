...
<Warning>
  Block-data RPC endpoints remain unavailable under Autobahn. Because the
  CometBFT block store height stays at `0`, `/block`, `/block_results`,
  `/commit`, and the EVM RPC endpoints that walk through them (for example
  `eth_getBlockByNumber` and `eth_gasPrice`) still fail on Autobahn nodes.
  Plan your monitoring and tooling accordingly until block-data reads are
  routed through an Autobahn-aware path.
</Warning>
